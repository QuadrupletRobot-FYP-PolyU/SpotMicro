...
class Config(metaclass=Singleton):
    ...
    def load_config(self):
        try:
            ...
            log.error(
                "Configuration file don't exist or is not a valid json, aborting.")
            sys.exit(1)

    def list_modules(self):
        log.info('Detected configuration for the modules: ' +
    ...
    def get(self, search_pattern):
        log.debug(search_pattern + ': ' +
                  str(jmespath.search(search_pattern, self.values)))
        return jmespath.search(search_pattern, self.values)

    def get_by_section_name(self, search_pattern):

        PCA9685 = 'motion_controller[*].servos[*].' + \
            str(search_pattern) + '[*].pca9685 | [0] | [0] | [0]'
        CHANNEL = 'motion_controller[*].servos[*].' + \
            str(search_pattern) + '[*].channel | [0] | [0] | [0]'
        MIN_PULSE = 'motion_controller[*].servos[*].' + \
            str(search_pattern) + '[*].min_pulse | [0] | [0] | [0]'
        MAX_PULSE = 'motion_controller[*].servos[*].' + \
            str(search_pattern) + '[*].max_pulse | [0] | [0] | [0]'
        REST_ANGLE = 'motion_controller[*].servos[*].' + \
            str(search_pattern) + '[*].rest_angle | [0] | [0] | [0]'

        PCA9685 = jmespath.search(PCA9685, self.values)

        PCA9685_ADDRESS = 'motion_controller[*].boards[*].pca9685_' + str(
            PCA9685) + '[*].address | [0] | [0] | [0]'
        PCA9685_REFERENCE_CLOCK_SPEED = 'motion_controller[*].boards[*].pca9685_' + str(
            PCA9685) + '[*].reference_clock_speed | [0] | [0] | [0]'
        PCA9685_FREQUENCY = 'motion_controller[*].boards[*].pca9685_' + str(
            PCA9685) + '[*].frequency | [0] | [0] | [0]'

        log.info('PCA9685_ADDRESS: ' +
                 str(jmespath.search(PCA9685_ADDRESS, self.values)))
        log.info('PCA9685_REFERENCE_CLOCK_SPEED: ' +
                 str(jmespath.search(PCA9685_REFERENCE_CLOCK_SPEED, self.values)))
        log.info('PCA9685_FREQUENCY: ' +
                 str(jmespath.search(PCA9685_FREQUENCY, self.values)))
